...
async def generatemp4(update, context):
    ...
    try:
        ...
        script = ask_ai(f"Write a short Hindi YouTube narration about {topic}. Return narration only.")

        mp3_file = f"{topic}_voice.mp3"

        gTTS(text=script, lang="hi", slow=False).save(mp3_file)

        await update.message.reply_text("✅ Voice generated. MP4 module installed (Phase23A).")
