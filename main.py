...
async def generatemp4(update, context):
    ...
    topic = " ".join(context.args) or "History"
    workdir = f"temp_{topic.replace(' ','_')}"
    ...
    try:
        ...
        script = ask_ai(f"""
Write a Hindi YouTube narration about:
...
""")
        ...
        audio_file = os.path.join(workdir, "voice.mp3")
        gTTS(text=script, lang="hi", slow=False).save(audio_file)
        ...
        for i in range(1, 6):
            prompt = ask_ai(f"Create cinematic image prompt #{i} for {topic}. Return only prompt.")
            url = "https://image.pollinations.ai/prompt/" + urllib.parse.quote(prompt[:300])
            response = requests.get(url, timeout=120)
        ...
        with open(os.path.join(workdir, "final.mp4"), "rb") as video:
            await update.message.reply_video(video=video, caption=f"🎬 Generated MP4\\n\\nTopic: {topic}")
