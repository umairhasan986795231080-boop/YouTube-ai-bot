...
async def imagebatch(update, context):
    ...
    await update.message.reply_text("🖼 Generating unique images...")
    ...
    for i, style in enumerate(scene_styles, start=1):
        try:
            prompt = ask_ai(f"""
Create a UNIQUE cinematic AI image prompt for {topic}.
...
""")
            ...
            url = "https://image.pollinations.ai/prompt/" + urllib.parse.quote(prompt[:300]) + f"?seed={seed}"
            ...
            await update.message.reply_text(f"Scene {i} failed: {e}")
