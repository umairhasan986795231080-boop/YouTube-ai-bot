...
async def generatevoice(update, context):
    ...
    try:
        ...
        script = ask_ai(
            f"""
Write a professional YouTube narration script about:
...
async def generatemp4(update, context):
    ...
    topic = " ".join(context.args) or "History"
    workdir = f"video_{topic.replace(' ','_')}"
    ...
    try:
        ...
        script = ask_ai(
            f"Write a short Hindi narration about {topic}. Return narration only."
        )

        mp3_file = os.path.join(workdir, "voice.mp3")
        ...
        for i in range(1, 6):
            url = "https://image.pollinations.ai/prompt/" + urllib.parse.quote(f"{topic} cinematic scene {i}")
            img_path = os.path.join(workdir, f"img{i}.jpg")
        ...
        subprocess.run([
            "ffmpeg","-y",
            "-loop","1","-t","3","-i", image_files[0],
            "-loop","1","-t","3","-i", image_files[1],
            "-loop","1","-t","3","-i", image_files[2],
            "-loop","1","-t","3","-i", image_files[3],
            "-loop","1","-t","3","-i", image_files[4],
            "-filter_complex",
        ...
        with open(final_video, "rb") as v:
            await update.message.reply_video(video=v, caption=f"🎬 {topic}")

    except Exception as e:
        await update.message.reply_text(f"❌ MP4 generation failed:\n{e}")
