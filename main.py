...
async def generatemp4(update, context):
    ...
    try:
        ...
        gTTS(text=script_text, lang="hi", slow=False).save(mp3_file)
        ...
        subprocess.run(
            ["ffmpeg","-y","-f","concat","-safe","0","-i","slides.txt",
             "-vsync","vfr","-pix_fmt","yuv420p","slideshow.mp4"],
            cwd=workdir,
            check=True
        )

        subprocess.run(
            ["ffmpeg","-y","-i","slideshow.mp4","-i","voice.mp3",
        ...
        with open(os.path.join(workdir, "final.mp4"), "rb") as vid:
